...
C         WRITE(9,*) J, N200, R200, M200, VDISP
 ...
 11      Q1 = RAN1(ISEMA)
         ...
         Z(I) = R(I)*COSTHETA
         RR = R(I)
C         VROT = VC2(CC,R200,V200,G,RR)
C        Gerando direções aleatórias novamente 
         COSTHETA = 2.*(RAN1(ISEMA)-0.5)
         SENTHETA = SQRT(1.0 - COSTHETA**2)
         PHI = 2.*PI*RAN1(ISEMA)
      VX(I) = SQRT(1./3.)*VDISP*GASDEV(ISEMA,ISEMB)*SENTHETA*COS(PHI)
      VY(I) = SQRT(1./3.)*VDISP*GASDEV(ISEMA,ISEMB)*SENTHETA*SIN(PHI)
      VZ(I) = SQRT(1./3.)*VDISP*GASDEV(ISEMA,ISEMB)*COSTHETA 
         WRITE(12,*) RR,VROT 
C-----------------------------------------------------
...
C         PARTE2 = SQRT(VX(I)**2 + VY(I)**2 + VZ(I)**2)
         ...
         WRITE(7,*) J,Y(I),Z(I),VLOS(I),DIST(I)
