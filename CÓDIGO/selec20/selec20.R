...
# leitura dados do aglomerado
myInputC = read.table("info.txt")
inputC = data.frame(cl = myInputC$V1, sigma = myInputC$V9, R200 = myInputC$V18, M200 = myInputC$V21)

# leitura dados galáxias do aglomerado
myInput = read.table("selec20.dat")
inputT = data.frame(ID = myInput$clusterID, ra = myInput$ra, dec = myInput$dec, zspec = myInput$zspec, Vel = myInput$Vel,flag = myInput$Flag, r = myInput$r)

for(cl in 1:max(inputT$ID)) {
	cluster_input = inputC[which(inputC$cl == as.integer(cl)),names(inputC) %in% c("sigma","R200","M200")]

	# galáxias membro flag = 0
	input <- inputT[which(inputT$ID == cl & inputT$flag == 0),names(inputT) %in% c("ID","ra","dec","zspec","Vel")]

	if(nrow(input) <= 20)
		print(paste(cl, " Número objetos: ",nrow(input)), sep="")
	if(nrow(input) > 20) {
		fileName = paste('cluster_M', cl, '.txt', sep="")

		file = paste("Aglomerado_rotacao M",cl, sep="")
		...
		if(length(gap_positions) == 0){
			...
			print(paste("MEDIANA ", cl, " = ", medVel[1]))
		}
