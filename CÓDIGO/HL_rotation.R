...
myInput = read.table("Soriano/teste/fort.8")
myInputVel = read.table("Soriano/teste/fort.7")
inputT = data.frame(ID = myInput$V1, ra = myInput$V2, dec = myInput$V3, zspec = myInput$V4, flag = myInput$V5)
inputV = data.frame(ID = myInputVel$V1, vlos = myInputVel$V4, flag = myInputVel$V6)

for(ID in 1:max(inputT$ID)) {
	
	input <- inputT[which(inputT$ID == ID & inputT$flag == 0),names(inputT) %in% c("ID","ra","dec","zspec")]
	input['vlos'] = subset(inputV[c("vlos")], inputV$ID == ID & inputV$flag == 0)
